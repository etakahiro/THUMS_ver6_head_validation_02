...
_implicit_1:
    ...
    →貫通
_implicit_2:
    陰解法,セグメントベースコンタクト法,頭部側の接触要素はshell要素,球をremesh
_implicit_3:
    陰解法,セグメントベースコンタクト法,頭部側の接触要素はshell要素,球はデフォルトのメッシュ
    →陽解法に比べてアワグラスは抑えられている
    →Total Energyが不安定になっている
    3.1:dtmaxを1.00000E-5に設定
    →解析の終盤で頭部と球の接触が貫通してしまった
    3.2:球の重心をyc=48.0に戻した(model_implicit/sphere_yoga3_mmton.k)
        dtmax=1.00000E-4
    →関係なし
    →要素タイプを変更したことによる影響
    3.3
_implicit_4:
    陰解法,要素タイプを変更なし,頭部と球の接触定義をソフトコンストレインに（デフォルト）
